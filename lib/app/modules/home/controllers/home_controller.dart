...
class HomeController {
  ...
  @observable
  ObservableMap<Genres, bool> genreFilterMap = Map<Genres, bool>.fromIterable(
      [],
        ...
        : movieList.where((Movie movie) {
            return movie.genreIds!.any((int id) =>
                genreFilterMap[_moviesStore.genres
                    .firstWhere((Genres genre) => genre.id == id)] ??
                false);
          }).toList();
  }
  ...
  Future<void> fetchGenres() async {
    ...
    try {
      ...
      genreFilterMap.addAll(Map<Genres, bool>.fromIterable(fetchedGenres,
          key: (item) => item, value: (_) => false).asObservable());
    } on DioError catch (_) {
      _moviesStore.loadingGenres = false;
      print("catch");
